...
from .services.thingspeak import fetch, ThingSpeakError
...
class ManagerAccess(ModuleAccess):
    def dispatch(self, request, *args, **kwargs):
        if request.user.is_authenticated and not usuario_tiene_cargo(request.user):
            return HttpResponse('Solo ADMIN o MANAGER pueden configurar los dendrómetros.', status=403)
        return super().dispatch(request, *args, **kwargs)
...
class FormMixin(ContextMixin):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, 'Configuración del dendrómetro guardada correctamente.')
        return response
...
class DatosView(ModuleAccess, View):
    def get(self, request, pk):
        ...
        try:
            ...
            selected = None
        return JsonResponse(analysis.payload(sensor, selected), json_dumps_params={'allow_nan': False})
...
class IniciarSyncView(ModuleAccess, View):
    def post(self, request, pk):
        ...
        try:
            ...
            return JsonResponse(sync.status(job) if job else {'estado': 'completa', 'progreso': 100})
        except ThingSpeakError as exc:
            return JsonResponse({'error': str(exc)}, status=400)
        except OperationalError:
            return JsonResponse({'error': 'La base está ocupada. Vuelve a intentar en unos segundos.'}, status=503)
...
class PasoSyncView(ModuleAccess, View):
    def post(self, request, pk, job_id):
        ...
        try:
            ...
            return JsonResponse({'error': 'La base está ocupada. La carga se puede reanudar; vuelve a intentar.'}, status=503)
...
class ProbarView(ManagerAccess, View):
    def post(self, request, pk):
        ...
        try:
            ...
            return JsonResponse({'ok': True, 'message': f'Conexión correcta. {len(rows)} lecturas numéricas recientes.' if rows else 'Conexión correcta; el campo aún no tiene lecturas numéricas.'})
        except ThingSpeakError as exc:
...
class ExportarView(ModuleAccess, View):
    def get(self, request, pk, tipo):
        ...
        response['Content-Disposition'] = f'attachment; filename="dendrometro_{sensor.pk}_{tipo}.csv"'
