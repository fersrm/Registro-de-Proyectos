...
class Dendrometro(models.Model):
    ...
    channel_id = models.PositiveIntegerField('Channel ID', validators=[MinValueValidator(1)])
    field_id = models.PositiveSmallIntegerField('Campo de señal', default=1, validators=[MinValueValidator(1), MaxValueValidator(8)])
    state_field_id = models.PositiveSmallIntegerField('Campo de estado', default=2, validators=[MinValueValidator(1), MaxValueValidator(8)])
    ...
    def config(self):
        ...
        return AppConfig(**values, channel_id=self.channel_id, field_id=self.field_id,
                         state_field_id=self.state_field_id, read_api_key=self.read_api_key)


class Lectura(models.Model):
    dendrometro = models.ForeignKey(Dendrometro, on_delete=models.CASCADE, related_name='lecturas')
    ...
    class Meta:
        ordering = ['created_at']
        constraints = [models.UniqueConstraint(fields=['dendrometro', 'created_at'], name='dendro_lectura_unica')]
        indexes = [models.Index(fields=['dendrometro', 'created_at'], name='dendro_sensor_fecha')]


class Sincronizacion(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    dendrometro = models.ForeignKey(Dendrometro, on_delete=models.CASCADE, related_name='sincronizaciones')
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=['dendrometro'], condition=models.Q(estado='pendiente'), name='dendro_sync_activa_unica')]
