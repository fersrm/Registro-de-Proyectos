...
from .thingspeak import ThingSpeakError, fetch
...
def status(job):
    ...
    percent = (
        100
        if job.estado == "completa"
        else min(99, round(job.segundos_completos / duration * 100))
    )
    return {
        "id": str(job.pk),
        "estado": job.estado,
...
def start(sensor, force=False):
    ...
    if (
        not force
        and sensor.ultima_sincronizacion
        and (now - sensor.ultima_sincronizacion).total_seconds()
        < sensor.config().refresh_seconds
    ):
        return (
            sensor.sincronizaciones.filter(estado="completa").order_by("-fin").first()
        )
    cfg = sensor.config()
    ...
    if (
        not force
        and sensor.sincronizado_desde
        and sensor.sincronizado_desde <= cutoff
        and sensor.ultima_sincronizacion
    ):
...
def step(job):
    ...
    acquired = (
        Sincronizacion.objects.filter(pk=job.pk, estado="pendiente")
    ...
    try:
        if sensor.revision != job.revision or not sensor.activo:
            raise ThingSpeakError(
                "La configuración cambió. Inicia una nueva sincronización."
            )
        ...
        with transaction.atomic():
            ...
            current = Sincronizacion.objects.filter(
                pk=job.pk, estado="pendiente", lease_token=token
            )
            ...
            if count >= 8000:
                if (end - begin).total_seconds() <= 2:
                    raise ThingSpeakError(
                        "Demasiadas entradas en el mismo segundo; no se puede asegurar una descarga completa."
                    )
                middle = (begin + (end - begin) / 2).replace(microsecond=0)
                job.rangos = [
                    [middle.isoformat(), end.isoformat()],
                    [begin.isoformat(), middle.isoformat()],
                ] + remaining
            else:
                Lectura.objects.bulk_create(
                    [Lectura(dendrometro=sensor, **row) for row in rows],
                    update_conflicts=True,
                    update_fields=["value", "estado", "entry_id"],
                    unique_fields=["dendrometro", "created_at"],
                    batch_size=400,
                )
            ...
            if not job.rangos:
                job.estado = "completa"
                earliest = (
                    min(sensor.sincronizado_desde, job.inicio)
                    if sensor.sincronizado_desde
                    else job.inicio
                )
                Dendrometro.objects.filter(pk=sensor.pk).update(
                    ultima_sincronizacion=job.fin,
                    sincronizado_desde=earliest,
                    ultimo_error="",
                )
