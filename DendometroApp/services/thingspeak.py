...
def fetch(sensor, start=None, end=None, results=8000):
    ...
    for key, value in [('start', start), ('end', end)]:
        if value is not None:
            params[key] = value.astimezone(dt_timezone.utc).strftime('%Y-%m-%d %H:%M:%S')
    try:
        response = requests.get(f'https://api.thingspeak.com/channels/{sensor.channel_id}/feeds.json', params=params, timeout=(5, 20))
        if response.status_code != 200:
            raise ThingSpeakError(f'ThingSpeak respondió HTTP {response.status_code}. Revisa el canal, la clave de lectura o intenta nuevamente.')
        payload = response.json()
    except (requests.RequestException, ValueError):
        raise ThingSpeakError('No se pudo consultar ThingSpeak. Revisa la conexión y vuelve a intentar.') from None
    if not isinstance(payload, dict) or not isinstance(payload.get('feeds'), list) or not isinstance(payload.get('channel'), dict):
        raise ThingSpeakError('ThingSpeak no entregó un canal válido. Revisa el Channel ID y la Read API Key.')
    ...
    for feed in payload['feeds']:
        ...
        if start and timestamp < start or end and timestamp > end:
            continue
        readings.append({'created_at': timestamp, 'entry_id': entry, 'value': value,
