...
def load(sensor):
    ...
    cutoff = timezone.now() - timedelta(
        hours=max(cfg.history_days * 24, cfg.realtime_hours)
    )
    raw = pd.DataFrame.from_records(
        sensor.lecturas.filter(created_at__gte=cutoff).values(
            "created_at", "entry_id", "value", "estado"
        ),
        columns=["created_at", "entry_id", "value", "estado"],
    )
    raw["created_at"] = pd.to_datetime(raw["created_at"], utc=True)
    analysis_raw = raw.loc[
        raw.created_at
        >= pd.Timestamp.now(tz="UTC") - pd.Timedelta(days=cfg.history_days)
    ]
    ...
    if latest:
        stats.latest_signal_um = float(
            input_to_um(pd.Series([latest.value]), cfg).iloc[0]
        )
    ...
    if cfg.sensor_in_maintenance:
        ...
        stats.alert_message = (
            "Todavía no se han recibido lecturas."
            if age is None
            else f"Última lectura hace {age:.0f} minutos. Comprueba el sensor y la conexión."
        )
        ...
        for item in parse_excluded_ranges(cfg.excluded_ranges_utc):
            ...
            operational = operational.loc[
                ~(
                    (operational.created_at >= item.start)
                    & (operational.created_at <= end)
                )
            ]
...
def interval_delta(proc, minutes):
    ...
    if len(current) < 2 or current.index[-1] - current.index[-2] != pd.Timedelta(
        minutes=minutes
    ):
        return None
...
def payload(sensor, selected=None):
    ...
    if not proc.empty:
        for sid, group in proc.groupby("session_id"):
            sessions.append(
                {
                    "id": int(sid),
                    "inicio": group.index[0].isoformat(),
                    "fin": group.index[-1].isoformat(),
                    "puntos": len(group),
                }
            )
            # Separar las series antes de reducir: ningún trazo cruza sesiones.
            values, reduced = records(
                group.reset_index()[["created_at", "um"]],
                limit=max(2, 6000 // max(1, stats.session_count)),
            )
        ...
        if selected in ids:
            ...
            if usable:
                chosen = usable[-1]
        detail, reduced_detail = records(
            proc.loc[proc.session_id == chosen].reset_index()
        )
    realtime = annotated
    if not realtime.empty:
        realtime = realtime.loc[
            realtime.created_at
            >= pd.Timestamp.now(tz="UTC") - pd.Timedelta(hours=cfg.realtime_hours)
        ]
    realtime_points, reduced_rt = records(realtime)
    minute_points = []
    if not realtime.empty:
        minute = (
            realtime.set_index("created_at")["um"]
            .resample("1min")
            .median()
            .reset_index()
        )
    ...
    if not proc.empty:
        current_interval_partial = proc.index[-1] + pd.Timedelta(
            minutes=cfg.aggregate_minutes
    ...
    return {
        "sensor": {
            ...
            "canal": sensor.channel_id,
            "activo": sensor.activo,
            "intervalo": cfg.aggregate_minutes,
            "refresh_seconds": cfg.refresh_seconds,
            "timezone": cfg.timezone_name,
            "maintenance": cfg.sensor_in_maintenance,
            "show_envelope": cfg.show_growth_envelope,
            "show_trend": cfg.show_slow_trend,
            "warn_low_um": cfg.warn_low_um,
            "warn_high_um": cfg.warn_high_um,
            "ultima_sincronizacion": sensor.ultima_sincronizacion.isoformat()
            if sensor.ultima_sincronizacion
            else None,
        ...
        "raw_count": len(raw),
        "sessions": sessions,
        "selected_session": chosen,
        "latest_session": sessions[-1]["id"] if sessions else None,
        "historic": historic,
        "detail": detail,
        "realtime": realtime_points,
        "minute_median": minute_points,
